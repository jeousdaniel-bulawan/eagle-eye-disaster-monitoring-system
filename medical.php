...
$sortBy = isset($_GET['sortBy']) ? htmlspecialchars($_GET['sortBy']) : 'record_id';
$order = isset($_GET['order']) ? strtoupper(htmlspecialchars($_GET['order'])) : 'ASC';
$allowedSortColumns = ['record_id', 'prescription', 'doctor_order', 'laboratory', 'medicine', 'daily_progress_notes', 'date'];
$allowedOrders = ['ASC', 'DESC'];

if (!in_array($sortBy, $allowedSortColumns)) {
    $sortBy = 'record_id'; // Default sorting column
}
...
$recordsPerPage = 10; // Adjust this as needed

// Calculate the total number of records
$totalRecordsQuery = "SELECT COUNT(*) FROM medical_records";
$totalRecordsResult = $pdo->query($totalRecordsQuery);
...
// Fetch records for the current page with ORDER BY
$query = "SELECT * FROM medical_records ORDER BY $sortBy $order LIMIT :limit OFFSET :offset";
try {
    ...
    echo '<div class="alert alert-danger">Error fetching medical records: ' . htmlspecialchars($e->getMessage()) . '</div>';
}
...
<head>
    ...
    <title>Bahay Kalinga | Medical Management System</title>
...
<div id="searchresult"></div>
        <div class="btn-group mb-3 mx-3">
                <a href="mecreate.php" class="btn btn-custom">New Medication</a>
            ...
            <tbody>
                <?php if (count($medicalRecords) > 0): ?>
                    <?php foreach ($medicalRecords as $record): ?>
                        <tr>
                            <td><?= 'MED-' . sprintf('%06d', $record['record_id']) ?></td>
                            <td><?= htmlspecialchars($record['prescription']) ?></td>
                            <td><?= htmlspecialchars($record['doctor_order']) ?></td>
                            <td><?= htmlspecialchars($record['laboratory']) ?></td>
                            <td><?= htmlspecialchars($record['medicine']) ?></td>
                            <td><?= htmlspecialchars($record['daily_progress_notes']) ?></td>
                            <td>
                            <div class="d-flex justify-content-start">
                                <a href="meedit.php?id=<?php echo $record['record_id']; ?>" class="btn btn-edit"><i class="bi bi-pencil-fill"></i></a>
                                <a href="medelete.php?id=<?php echo $record['record_id']; ?>" class="btn btn-delete" onclick="return confirm('Are you sure you want to delete this medication?');"><i class="bi bi-trash-fill"></i></a>
                                </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
